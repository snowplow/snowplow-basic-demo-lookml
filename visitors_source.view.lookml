...
- view: visitors_source
  derived_table:
    sql: |
      ...
      FROM (
        SELECT -- Select campaign and referer data from the earliest row (using dvce_tstamp)
          ...
          a.refr_urlhost,
          a.refr_urlpath,
          RANK() OVER (PARTITION BY a.domain_userid
            ORDER BY a.mkt_source, a.mkt_medium, a.mkt_term, a.mkt_content, a.mkt_campaign,
              a.refr_source, a.refr_medium, a.refr_term, a.refr_urlhost, a.refr_urlpath) AS rank
    ...
    sql_trigger_value: SELECT COUNT(*) FROM ${visitors_landing_page.SQL_TABLE_NAME}  # Generate this table after visitors_landing_page
